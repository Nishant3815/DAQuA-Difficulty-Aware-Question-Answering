...
def get_query2vec(query_encoder, tokenizer, args, batch_size=64, silent=False):
...
def load_qa_pairs(data_path, args, q_idx=None, draft_num_examples=100, shuffle=False, multihop=False):
    ...
    if args.draft:
        ...
        final_answers = np.array(final_answers)[:draft_num_examples].tolist()
        final_titles = np.array(final_titles)[:draft_num_examples].tolist()
