...
def train_query_encoder(args, save_path, mips=None, init_dev_acc=None):
    ...
    if not args.skip_warmup:
        ...
        for ep_idx in range(int(args.num_firsthop_epochs)):
            ...
            if not args.skip_warmup_dev_eval:
                ...
                eval_res = evaluate(new_args, mips=mips, firsthop=True,
    ...
    if not args.warmup_only:
        ...
        if not args.skip_warmup:
            ...
            new_args.top_k = 10
            new_args.test_path = args.dev_path
            dev_em, dev_f1, dev_emk, dev_f1k = evaluate(new_args, mips=mips, multihop=True,
            ...
            for step_idx, (q_ids, levels, questions, answers, titles, outs, final_answers, final_titles) in enumerate(pbar):
                ...
                for i, q in enumerate(questions):
                    ...
                    if levels[i] == 'easy':
                        ...
                        upd_q_id = q_ids[i] + "_uc"  # unchanged
                        upd_level = levels[i]
                        upd_evidence = ''
                        upd_evidence_title = ''
                        ...
                        # Append question with gold SUP sentences (THIS LOGIC ONLY HERE; NOT IN query_ftune.py)
                        if args.upd_sent_evd:
                            upd_q_id = q_ids[i] + f"_g"  # gold
                            upd_level = levels[i]
                            upd_evidence = ' '.join(answers[i])
                            upd_evidence_title = ' '.join(titles[i])
                            upd_answer = final_answers[i]
                            upd_answer_title = final_titles[i]
                            upd_queries.append(
                                (upd_q_id, upd_level, q, upd_evidence, upd_evidence_title, upd_answer, upd_answer_title)
...
if __name__ == '__main__':
    ...
    if args.run_mode == 'train_query':
        ...
        if not args.skip_final_eval:
            ...
            for split in paths:
                if args.load_warmup_dir is not None:
                    logger.info(
                        f"Final: Evaluating 1st:{args.load_warmup_dir}, 2nd:{args.load_dir} on the {split.upper()} set")
                else:
        ...
        for split in paths:
            if args.load_warmup_dir is not None:
                logger.info(f"Evaluating 1st:{args.load_warmup_dir}, 2nd:{args.load_dir} on the {split.upper()} set")
            else:
