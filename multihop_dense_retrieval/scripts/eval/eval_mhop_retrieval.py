...
python scripts/eval/eval_mhop_retrieval.py \
    ...
    models/q_encoder.pt \
    --hop1 intermediate/single_hop_eval.json
    --batch-size 100 \
    --beam-size 1 \
    --topk 1 \
    --shared-encoder \
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hop1', type=str, default=None)
    ...
    if args.hnsw:
        ...
        if args.gpu:
            ...
            # changed from 6 -> 0 because invalid device error in unity
    ...
    logger.info("Reading 1st Hop Phrase Retrievals and Encoding Query for Hop 2")

    hop1_phrases = args.hop1
    with open(hop1_phrases, 'r') as data_file:
    ...
    for b_start in tqdm(range(0, len(questions), args.batch_size)):
        with torch.no_grad():
            ...
            logger.info("D_ shape:" + str(D_.shape))
            logger.info("I_ shape:" + str(I_.shape))
            D_ = D_.reshape(bsize, args.beam_size, args.beam_size)
            I_ = I_.reshape(bsize, args.beam_size, args.beam_size)

            # aggregate path scores
            # not aggregating as getting 1st hop predictions from DensePhrases 
